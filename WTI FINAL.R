...
## Data Exploration ------------------------------------------------------------
ggplot(WTI_price, aes(x = Date, y = WTI)) +
  geom_line(color = "blue") + 
  theme_minimal() +  
  labs(
    title = "Daily WTI Crude Oil Closing Price",
    x = "Date",
    y = "Price"
  ) +
  theme(
  ...
  guides(
    color = guide_legend("WTI Daily Prices") 
  )

qqnorm(WTI_price$WTI,main="WTI SPOT PRICES")
qqline(WTI_price$WTI)

## Obtain Returns --------------------------------------------------------------
WTI_price$Returns <- 100 * c(NA, diff(log(WTI_price$WTI)))

# Plotting WTI Returns
wti_return_p <- ggplot(WTI_price, aes(x = Date, y = Returns)) +
  geom_line(color = "blue", na.rm = TRUE) +
  theme_minimal() +
  labs(title = "Daily WTI Crude Oil Returns", x = "Date", y = "Returns") +
...
meanreturn <- mean(na.omit(WTI_price$Returns), na.rm = TRUE)
sdreturn <- sd(na.omit(WTI_price$Returns), na.rm = TRUE)

# Plot the histogram and  normal distribution
hist(WTI_price$Returns, breaks = 40, freq = FALSE, main = "Log Return Histogram", xlab = "Log Returns", col = "lightblue")
curve(dnorm(x, mean = meanreturn, sd = sdreturn), add = TRUE, col = "red", lwd = 2)

# Plotting the density of log returns
plot(density(na.omit(WTI_price$Returns)), main = "Log Return Empirical Distribution", 
     xlab = "Log Returns", ylab = "Density", col = "blue")
curve(dnorm(x, mean = meanreturn, sd = sdreturn), 
      from = min(na.omit(WTI_price$Returns)), 
      to = max(na.omit(WTI_price$Returns)), 
      add = TRUE, col = "red", lwd = 2)

# Plotting the density of log returns
plot(density(na.omit(WTI_price$Returns)), 
...
WTI_price$SquaredReturns <- WTI_price$Returns^2

# Plotting the squared log returns
ggplot(WTI_price, aes(x = Date, y = SquaredReturns)) +
  geom_line(color = "red", na.rm = TRUE) +  
  theme_minimal() + 
  labs(
    title = "Daily Squared Log Returns of WTI Crude Oil",
    x = "Date",
...
# Descriptive stats OPEC Table 1:-----------------------------------------------
table(OPEC_meetings$Decision)
sum(OPEC_meetings$Meeting == "Ordinary OPEC Conference")
sum(OPEC_meetings$Meeting == "Extraordinary OPEC Conference")

extra_meetings <- OPEC_meetings[OPEC_meetings$Meeting == "Extraordinary OPEC Conference",]
ordinary_meetings <- OPEC_meetings[OPEC_meetings$Meeting == "Ordinary OPEC Conference",]
sum(extra_meetings$Decision == "cut")
sum(extra_meetings$Decision == "maintain")
sum(extra_meetings$Decision == "hike")

sum(ordinary_meetings$Decision == "cut")
sum(ordinary_meetings$Decision == "maintain")
sum(ordinary_meetings$Decision == "hike")

# Descriptive Stats: Table 2:---------------------------------------------------
wti_returns <- na.omit(WTI_price$Returns)

# Calculate mean
mean_returns <- mean(wti_returns)

# Calculate variance
variance_returns <- var(wti_returns)

# Calculate standard deviation
std_dev_returns <- sd(wti_returns)

# Calculate skewness
skewness_returns <- skewness(wti_returns)

# Calculate kurtosis
kurtosis_returns <- kurtosis(wti_returns)
...
# Normality test ---------------------------------------------------------------
qqnorm(WTI_price$Returns,main="WTI RETURNS")
qqline(WTI_price$Returns)

# Jarque-Bera Test
jarque.bera.test(wti_returns)

# Ljung-Box Test
Box.test(wti_returns, type = "Ljung-Box")
Box.test(wti_returns, lag = 15, type = "Ljung-Box")

# Testing for Stationary -------------------------------------------------------
# Augmented Dickey-Fuller Test
adf.test(wti_returns, alternative = "stationary")

# Phillips-Perron Test
ur.pp(wti_returns, type = "Z-alpha")

result <- ur.pp(wti_returns, type='Z-tau')
cat("Test Statistic:", result@teststat, "\n")
cat("Critical Values: 1%:", result@cval[3], "\n")

# KPSS Test
kpss.test(wti_returns)

## Heteroskedasticity test -----------------------------------------------------
## ARCH LM test ----------------------------------------------------------------
wti_returns_ts <- ts(wti_returns) 

arch.test(wti_returns_ts, arch = "box", alpha = 0.05) # presence of heterocasdicity.

## Plot Fig1:-------------------------------------------------------------------
p_wti <- ggplot(WTI_price, aes(x = Date, y = WTI)) +
  ...
  labs(title = "Spot Price of WTI", color = "Decision Type") +
  theme_minimal() +
...
hurst_result <- hurstexp(wti_returns_ts)

## Hurt exponent through regression --------------------------------------------
L <- length(wti_returns)
...
for(k in 1:length(vLs)) {
  ...
  for(i in 1:Ns) {
    # i-th sub series
    x_is <- wti_returns[(Ls*(i-1)+1):(Ls*i)]
...
summary(log_fit)
## GPH Test --------------------------------------------------------------------
gph_result<- fdGPH(wti_returns_ts, bandw.exp = 0.45)
print(gph_result)
d_hat <- gph_result$d
...
## alpha = 0.50 ----------------------------------------------------------------
gph_result <- fdGPH(wti_returns_ts, bandw.exp = 0.5)
print(gph_result)
d_hat <- gph_result$d
sd_as <- gph_result$sd.as
t_statistic <- d_hat / sd_as
...
gph_result <- fdGPH(wti_returns_ts, bandw.exp = 0.55)
print(gph_result)
d_hat <- gph_result$d
sd_as <- gph_result$sd.as
t_statistic <- d_hat / sd_as
...
log_returns <- na.omit(wti_returns) 
...
################################################################################
WTI_price$SquaredReturns <- WTI_price$Returns^2
wti_squared_returns <- na.omit(WTI_price$SquaredReturns)
wti_squared_returns_ts <- ts(wti_squared_returns)

## Hurst Exponent --------------------------------------------------------------
## Hurt exponent through regression --------------------------------------------
L <- length(wti_squared_returns)

# Minimum length of sub series = around 8
vLs <- round(L/2^(0:floor(log2(L/8)))) # length of sub series
...
for(k in 1:length(vLs)) {
  ...
  for(i in 1:Ns) {
    x_is <- wti_squared_returns[(Ls*(i-1)+1):(Ls*i)]
...
## Lo's modified R/S test ------------------------------------------------------
L <- length(wti_squared_returns_ts)
vLs <- round(L/2^(0:floor(log2(L/8)))) 
df_rs <- data.frame(QT=NA, Ls=vLs)
...
## GPH test --------------------------------------------------------------------
gph_result<- fdGPH(wti_squared_returns_ts, bandw.exp = 0.45)
d_hat <- gph_result$d
sd_as <- gph_result$sd.as
t_statistic <- d_hat / sd_as
...
## alpha = 0.5 -----------------------------------------------------------------
gph_result <- fdGPH(wti_squared_returns_ts, bandw.exp = 0.5)
d_hat <- gph_result$d
sd_as <- gph_result$sd.as
t_statistic <- d_hat / sd_as
...
## alpha = 0.55 ----------------------------------------------------------------
gph_result <- fdGPH(wti_squared_returns_ts, bandw.exp = 0.55)
d_hat <- gph_result$d
sd_as <- gph_result$sd.as
t_statistic <- d_hat / sd_as
...
## Merge datas -----------------------------------------------------------------
WTI_price$Returns <- 100 * c(NA, diff(log(WTI_price$WTI)))
merged_df <- merge(WTI_price, Dollar_index, by="Date", all.x=TRUE)
...
# Handdle NA value for wti returns
merged_df$Returns <- na_locf(merged_df$Returns)
merged_df$Squared_Returns <- na_locf(merged_df$Squared_Returns)
...
wti_xts <- xts(merged_df$Returns, order.by = merged_df$Date)

# whole data:
auto_fit <- auto.arima(wti_xts, stationary = TRUE, stepwise = T, approximation = FALSE, ic = "aic")

summary(auto_fit)  # ARIMA(2,0,3)
checkresiduals(auto_fit)

# Extracting the returns data from the xts object as a numeric vector
returns_data <- coredata(wti_xts)
wti_returns <- returns_data  

# Get the ACF values without plotting
acf_values <- acf(wti_returns, lag.max = 30, plot = FALSE)

# acf_values$acf will contain the autocorrelation coefficients
...
# Print autocorrelation values at lags 
print(paste("Autocorrelation at lag 1:", lag_1_autocorr)) # not significant autocorrelation
print(paste("Autocorrelation at lag 2:", lag_2_autocorr)) # significant autocorrelation
print(paste("Autocorrelation at lag 3:", lag_3_autocorr)) # slightly outside 
...
merged_df <- merged_df %>%
  mutate(
    rt_lag2 = lag(Returns, 2),
    rt_lag3 = lag(Returns, 3),
...
for (i in 1:n_samples) {
  ...
  # Fit the model
  model <- lm(Returns ~ rt_lag2 + rt_lag3 + rt_lag4 + cut_modified + maintain_modified + hike_modified, data = modified_data)
...
model <- lm(Returns ~ rt_lag2 + rt_lag3 + rt_lag4 + cut + hike + maintain +SP500_r + dxy_r, data = merged_df)
...
model2 <- lm(Returns ~ rt_lag2 + rt_lag3 + rt_lag4 + cut_modified + hike_modified + maintain_modified +SP500_r+ dxy_r, data = modified_data)
...
model3 <- lm(Returns ~ rt_lag2 + rt_lag3 + rt_lag4 + cut_modified + hike_modified +
               maintain_modified + SP500_r +dxy_r, data = modified_data)

# Summary of the model to check coefficients and model statistics
summary(model3)
AIC(model3)
#everything is significant other than mantain_modified.

model4 <- lm(Returns ~ rt_lag2 + rt_lag3 + rt_lag4 + cut_modified + hike_modified +
...
# DETERMINING MEAN EQUATION ----------------------------------------------------
InSampleData  = wti_xts[1:4444]
OutSampleData = wti_xts[4445:5556]

covariates <- merged_df[, c("cut", "maintain", "hike")]
covariates_matrix <- data.matrix(covariates)
covariates_matrix_aligned <- covariates_matrix[1:4444, ]
...
## Arma(2,2) mode with covariates ----------------------------------------------
Arima(InSampleData, xreg=covariates_matrix_aligned, order=c(2,0,2))
...
norm_spec <- ugarchspec(variance.model = list(model = "fiGARCH", garchOrder = c(1, 1),external.regressors = external_regressor),
                   mean.model = list(armaOrder = c(2, 2), arfima = T, include.mean = TRUE),
...
norm_garch_fit <- ugarchfit(spec = norm_spec, data = wti_xts, out.sample = T_test)
show(norm_garch_fit)
...
std_garch_fit <- ugarchfit(spec = std_spec, data = wti_xts, out.sample = T_test)
show(dtf_garch_fit)
...
sstd_garch_fit <- ugarchfit(spec = sstd_spec, data = wti_xts, out.sample = T_test)
show(sstd_garch_fit)
...
dummies <- as.matrix(merged_df[, c("hike","SP500_r")])
...
garch_fit <- ugarchfit(spec = spec, data = wti_xts, out.sample = T_test)
show(garch_fit)
...
fit_garch_and_get_aic <- function(data, params) {
  ...
  external_regressors <- as.matrix(modified_data[, c("hike_anticipated", "SP500_r")])
  spec <- ugarchspec(mean.model = list(armaOrder = c(2,2), include.mean = TRUE),
  ...
  fit <- tryCatch({
    ...
    return(list(aic = Inf))  # Return Inf AIC on error
  })
...
all_params <- expand.grid(s1_hike = s1_range, s2_hike = s2_range, s3_hike = s3_range)

# Convert the generated data frame to a list of lists (if necessary)
generate_all_params <- function(params_df) {
  apply(params_df, 1, function(x) list(s1_hike = x['s1_hike'], s2_hike = x['s2_hike'], s3_hike = x['s3_hike']))
}

all_params_list <- generate_all_params(all_params)

# Export necessary objects to the cluster
clusterExport(cl, varlist = c("modify_all_dummies_reverse", "fit_garch_and_get_aic", "all_params_list", "merged_df"))

# Performing optimization
results <- foreach(param = all_params_list, .packages = c("rugarch", "dplyr")) %dopar% {
...
dummies <- as.matrix(modified_data[, c("hike_anticipated", "SP500_r")])
...
fit <- ugarchfit(spec = sstd_spec, data = wti_xts, out.sample = T_test)
fit <- ugarchfit(spec = sstd_spec, data = InSampleData)
show(fit)
...
## VAR Backtesting #############################################################
roll = ugarchroll(spec = sstd_spec , data = wti_xts[1:4444] , n.start = 3944,
                  refit.window = 'moving',calculate.VaR = TRUE,
                  VaR.alpha = c(0.05, 0.01))

# For 99% confidence level
report_99 <- report(roll, type = "VaR", VaR.alpha = 0.01, conf.level = 0.95)

# For 95% confidence level
report_95 <- report(roll, type = "VaR", VaR.alpha = 0.05, conf.level = 0.95)

# Extracting VaR and actual values
if ("forecast" %in% slotNames(roll) && !is.null(roll@forecast$VaR)) {
  EbacktestVaR_5 <- zoo(roll@forecast$VaR[, "alpha(5%)"], order.by = as.Date(rownames(roll@forecast$VaR)))
  EbacktestVaR_1 <- zoo(roll@forecast$VaR[, "alpha(1%)"], order.by = as.Date(rownames(roll@forecast$VaR)))
  Ebackactual <- zoo(roll@forecast$VaR[, "realized"], order.by = as.Date(rownames(roll@forecast$VaR)))
  
  # Plotting the backtest results
  plot(Ebackactual, type = "b", main = "WTI VaR Backtesting for 5% and 1%", xlab = "Date", ylab = "Return/VaR in percent",
...
## Expected Shortfall ----------------------------------------------------------
fit = ugarchfit(sstd_spec, data = wti_xts[1:4444])
spec2 = sstd_spec
setfixed(spec2)<-as.list(coef(fit))
filt = ugarchfilter(spec2, wti_xts[4445:5556])
actual = wti_xts[4445:5556]
...
print(ESTest(0.01, actual, ES, VaR, boot = TRUE))
...
# Calculate VaR for both long and short positions
alpha <- 0.01
skewness = coef(fit)["skew"]
shape = coef(fit)["shape"]
VaR_L <- fitted(filt) - sigma(filt) * qdist("sstd", p=alpha, mu=0, sigma=1, skew=skewness, shape=shape)
VaR_S <- fitted(filt) + sigma(filt) * qdist("sstd", p=1-alpha, mu=0, sigma=1, skew=skewness, shape=shape)

# Compute losses
losses_long = actual - VaR_L
losses_short = VaR_S - actual

# Compute ESF1 and ESF2
ESF1_long = mean(losses_long[losses_long > 0])
ESF1_short = mean(losses_short[losses_short > 0])
mean_VaR_L = mean(VaR_L[losses_long > 0])
mean_VaR_S = mean(VaR_S[losses_short > 0])
ESF2_long = ifelse(mean_VaR_L != 0, ESF1_long / mean_VaR_L, NA)
ESF2_short = ifelse(mean_VaR_S != 0, ESF1_short / mean_VaR_S, NA)

# Output results
print(paste("ESF1 Long: ", ESF1_long))
print(paste("ESF2 Long: ", ESF2_long))
print(paste("ESF1 Short: ", ESF1_short))
print(paste("ESF2 Short: ", ESF2_short))
...
forecast_dates <- index(wti_xts)[4445:5555]
...
actual_log_returns <- wti_xts[forecast_dates]
...
p_wti <- ggplot(WTI_price, aes(x = Date, y = WTI)) +
  ...
  scale_y_continuous(limits = c(0, 150), name = "Spot price/Dollars per Barrel") +
  scale_color_manual(values = c("Maintain" = "darkgray", "Decrease" = "red", "Increase" = "#4CBB17")) +
  labs(title = "Spot Price of WTI - Extraordinary OPEC Conference Decisions", color = "Decision Type") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), plot.title = element_text(hjust = 0.5))
